...
async function enrich(instance, index, total) {
  if (index > 0 && index % 25 === 0) setStatus(`Analysing ${index}/${total}...`);
  let template = {};
  let entry = null;
...
async function fetchInstances() {
  ...
  try {
    ...
    throw buildMissingIndexError("Compliance records", error, "Failed to load records.");
  }
...
function applyClientFilters() {
  ...
  return enrichedRecords.filter((row) => {
    ...
    if (userText && !String(row.completedBy || "").toLowerCase().includes(userText)) return false;
    if (search) {
      const text = [
        row.storeName,
        row.storeId,
...
function render(records) {
  ...
  const backUrl = `../admin/compliance-records.html?${backParams}`;
  ...
  pageRecords.forEach((row) => {
    ...
    viewLink.href = `view-entry.html?instanceId=${encodeURIComponent(row.instanceId)}&templateId=${encodeURIComponent(row.templateId)}&storeId=${encodeURIComponent(row.storeId)}&date=${encodeURIComponent(row.date)}&back=${encodeURIComponent(backUrl)}`;
  ...
  try {
    ...
    if (exportedCount > 0) {
      await writeUserActivity({
        ...
        summary: `Exported ${exportedCount} compliance record PDF(s)`,
        storeId: storeFilter.value || "",
        department: departmentFilter.value || "",
        section: sectionFilter.value || "",
        metadata: {
          source: "admin_compliance_records",
          mode: label,
...
async function loadAndRender() {
  ...
  try {
    ...
    if (limitHint && raw.length >= FETCH_BATCH_SIZE * MAX_FETCH_BATCHES) {
      limitHint.style.display = "block";
      limitHint.textContent = `Loaded the first ${raw.length} records in this range. Narrow the date range or store for faster, more precise results.`;
    }
...
async function init() {
  ...
  if (params.get("store")) storeFilter.value = params.get("store");
  ...
  if (params.get("user")) userFilter.value = params.get("user");
  ...
  if (!openedSectionLogged) {
    ...
    await writeUserActivity({
      ...
      summary: "Opened Admin Compliance Records",
      storeId: storeFilter.value || "",
      department: initialDepartment || departmentFilter.value || "",
      section: initialSection || sectionFilter.value || "Compliance Records",
      metadata: {
        source: "admin_compliance_records"
      }
    });
